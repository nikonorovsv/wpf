 ...
 */
class ThemeSupportsDefiner extends Observer
{
    /**
     * @param App $app
     */
    public function doUpdate(App $app)
    {
        $update = function () use ($app) {
          ...
          __("'Supports' property in '*.config.json' must be an array.", 'wpf'));
            }
            foreach ($app->supports as $args) {
                if (!is_array($args)) {
                    throw new ConfigException(
            __("All items of 'supports' property in '*.config.json' must be arrays too.", 'wpf'));
                }
